...
const std::uint8_t BitmapFont8x16::glyphs[BitmapFont8x16::kGlyphCount][BitmapFont8x16::kHeight] = {
...
bool BitmapFont::valid() const
{
	return true;
}

int BitmapFont::sizePx() const
{
	return kHeight;
}

int BitmapFont::lineHeight() const
{
	return kHeight;
}

int BitmapFont::ascender() const
{
	return kHeight;
}

bool BitmapFont::glyph(int codepoint, Glyph *out) const
{
	if (!out) return false;
	if (codepoint < kFirstCodepoint || codepoint > kLastCodepoint) codepoint = '?';
	Glyph glyph{};
	glyph.codepoint = codepoint;
	glyph.width = kWidth;
	glyph.height = kHeight;
	glyph.advance = kWidth;
	glyph.bearingX = 0;
	glyph.bearingY = kHeight;
	*out = glyph;
	return true;
}

std::uint8_t BitmapFont::coverage(const Glyph &glyph, int row, int col) const
{
	if (row < 0 || row >= kHeight || col < 0 || col >= kWidth) return 0;
	const std::uint8_t *rows = rowsForCodepoint(glyph.codepoint);
	return (rows[row] & (0x80 >> col)) ? 255 : 0;
}

const std::uint8_t *BitmapFont::glyphRows(int codepoint) const
{
	return rowsForCodepoint(codepoint);
}

const BitmapFont8x16 &BitmapFont8x16::instance()
{
	static const BitmapFont8x16 font;
	return font;
}

const std::uint8_t *BitmapFont8x16::rowsForCodepoint(int codepoint) const
{
